...
public class Searcher implements FilenameFilter {
    ...
    public InputStream[] search(String packageName) throws IOException {
	PathMatchingResourcePatternResolver resourcePatternResolver = new PathMatchingResourcePatternResolver();
	Resource[] resources = resourcePatternResolver.getResources("classpath:" + packageName + "/*.aiml");
	InputStream[] inputStreams = new InputStream[resources.length];
	int resourcesInex = 0;
	for (Resource resource : resources) {
	    inputStreams[resourcesInex] = resource.getInputStream();
	    resourcesInex++;
	}
	return inputStreams;
    }
