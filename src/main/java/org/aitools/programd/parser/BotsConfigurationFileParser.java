...
public class BotsConfigurationFileParser {
    ...
    private static final Namespace NS = Namespace.getNamespace("x",
	    NAMESPACE_URI);
	...
	public static SubstitutionType get(String name) {
	    ...
	    throw new IllegalArgumentException(String.format(
		    "\"%s\" is not a valid substitution type.", name));
	}
    }
	...
	if (root.getName().equals("bots")) {    
	    for (Element bot : (List<Element>) root.getChildren("bot", rootNamespace)) {
		loadBot(bot);
	    }
	    ...
	    throw new IllegalArgumentException(String.format(
		    "Invalid bot config file \"%s\".", path));
	}
    }
	...
	if (element.getAttribute("href") != null) {
	    parse(JDOM
		    .contextualize(element.getAttributeValue("href"), element));
	} else {
		...
		if (bots.containsKey(botid)) {
		    this._logger.warn(String.format(
			    "Bot \"%s\" has already been configured.", botid));
		    return;
		}
		Bot bot = new Bot(botid, this._core.getSettings());

		this._logger.info(String.format("Configuring bot \"%s\".",
			botid));
		bots.put(botid, bot);
		...
		int previousDuplicateCount = graphmapper
		...
		this._logger.info(String.format(
			"%,d categories loaded in %.4f seconds.",
			graphmapper.getCategoryCount() - previousCategoryCount,
			time / 1000.00));
		this._logger.info(graphmapper.getCategoryReport());

		int dupes = graphmapper.getDuplicateCategoryCount()
			- previousDuplicateCount;
		if (dupes > 0) {
		    this._logger
			    .warn(String
				    .format("%,d path-identical categories were encountered, and handled according to the %s merge policy.",
					    dupes, this._core.getSettings()
						    .getMergePolicy()));
		}
	    }
	}
     ...
     * 
     * @param bot
     *            the bot object into which to load data
     * @param parent
     *            the parent "bot" element
     * @param name
     *            the name of the child element from which to load config data;
     *            is used to determine method name to call to load config data
     */
    protected void loadConfig(Bot bot, Element parent, String name) {
	Element child = parent.getChild(name, NS);
	if (child != null) {
	    String methodName = String.format("load%s%s", name.substring(0, 1)
		    .toUpperCase(), name.substring(1).replaceAll("\\W", ""));
	    Method method;
	    try {
		method = this.getClass().getDeclaredMethod(methodName,
			new Class[] { Bot.class, Element.class });
	    } catch (SecurityException e) {
		throw new DeveloperError(String.format(
			"Cannot access method %s", methodName), e);
	    } catch (NoSuchMethodException e) {
		throw new DeveloperError(String.format("No such method %s",
			methodName), e);
	    }
	    try {
		if (child.getAttribute("href") != null) {
		    method.invoke(
			    this,
			    bot,
			    getDocRoot(JDOM.contextualize(
				    child.getAttributeValue("href"), child)));
		} else {
		    method.invoke(this, bot, child);
		}
	    } catch (InvocationTargetException e) {
		throw new DeveloperError(String.format(
			"Could not invoke method %s", methodName), e);
	...
	for (Element property : (List<Element>) element.getChildren("property",
		NS)) {
	    bot.setPropertyValue(property.getAttributeValue("name"),
	...
	for (Element predicate : (List<Element>) element.getChildren(
		"predicate", NS)) {
	...
	for (Element substitutionTypes : (List<Element>) element.getChildren()) {
	    SubstitutionType type = SubstitutionType.get(substitutionTypes
		    .getName());

	    for (Element substitution : (List<Element>) substitutionTypes
		    .getChildren()) {
		...
		Pattern pattern;
		try {
		    pattern = Pattern.compile(find, Pattern.CANON_EQ
			    | Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
		} catch (PatternSyntaxException e) {
		    throw new UserError(String.format(
			    "Invalid substitution pattern \"%s\".", find), e);
		}
		...
		case GENDER:
		    bot.addSubstitution(
			    org.aitools.programd.processor.aiml.GenderProcessor.class,
			    pattern, replace);
		    break;
		case PERSON:
		    bot.addSubstitution(
			    org.aitools.programd.processor.aiml.PersonProcessor.class,
			    pattern, replace);
		    break;
		case PERSON2:
		    bot.addSubstitution(
			    org.aitools.programd.processor.aiml.Person2Processor.class,
			    pattern, replace);
		    break;
		}
	    }
	...
	for (Element splitter : (List<Element>) element.getChildren("splitter",
		NS)) {
	...
	for (Element listenerElement : (List<Element>) element.getChildren(
		"listener", NS)) {
	    // Enabled?
	    boolean enabled = false;
	    try {
		enabled = listenerElement.getAttribute("enabled")
			.getBooleanValue();
	    ...
	    for (Element parameter : (List<Element>) listenerElement
		    .getChildren()) {
		parameters.put(parameter.getAttributeValue("name"),
	    ...
	    Listener listener = Classes.getSubclassInstance(Listener.class,
		    classname, "listener", this._core, bot, parameters);
	    ...
	    // Start listener
	    this._core.getManagedProcesses().start(listener,
		    String.format("%s : %s", classname, bot.getID()));

	    this._logger.info(String.format(
		    "Started \"%s\" listener for bot \"%s\".", classname,
		    bot.getID()));
	}
    }
	...
	try {
	    docURL = URLTools
		    .createValidURL(element.getDocument().getBaseURI());
	} catch (FileNotFoundException e) {
	    throw new DeveloperError(
		    "Could not get bot config document URL when setting up testing.",
		    e);
	}
	bot.setTestSuitePathspec(URLTools.getURLs(
		element.getChildText("test-suite-path", NS), docURL));
	bot.setTestReportDirectory(URLTools.contextualize(docURL,
		element.getChildText("report-directory", NS)));
    }

    @SuppressWarnings("unchecked")
    protected void loadAIML(Bot bot, Element element) {
	for (Element learn : (List<Element>) element.getChildren("learn", element.getNamespace())) {
	    this._core.load(JDOM.contextualize(learn.getText(), element),
		    bot.getID());
	}
    }

    /**
     * A common method used in this class for loading a document and getting its
     * root element.
     * 
     * @param path
