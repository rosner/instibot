...
abstract public class AbstractGraphmapper implements Graphmapper {
	...
	this._categoryLoadNotifyInterval = settings
		.getCategoryLoadNotificationInterval();
	this._aimlNamespaceURI = settings.getAIMLNamespaceURI().toString();
    }

    /**
     * @see org.aitools.programd.graph.Graphmapper#load(java.net.URL,
     *      java.lang.String)
     */
	...
	if (isAlreadyLoaded(path)) {
		...
		if (this._logger.isDebugEnabled()) {
		    this._logger
			    .debug(String
				    .format("Reloading \"%s\" for \"%s\" (is that what you wanted?).",
					    path, botid));
		}
		unload(path, bot);
		doLoad(path, botid);
		...
		if (this._logger.isDebugEnabled()) {
		    this._logger
			    .debug(String
				    .format("Graphmaster has already loaded \"%s\" for some other bot.",
					    path));
		}
		...
		this._logger.info(String.format("Loading %s....",
			URLTools.unescape(path)));
	    }
	    doLoad(path, botid);
	...
	try {
	    ...
	    XMLStreamReader xmlStreamReader = xmlInputFactory.createXMLStreamReader(path.openStream());
	    XMLReader reader = StaxUtils.createXMLReader(xmlStreamReader);
	    
//	    XMLReader reader = SAX.getReader(this._logger);
	    
	    AIMLReader handler = new AIMLReader(this, path,
		    this._core.getBot(botid));
	    ...
	    this._logger.warn(
		    String.format("Error reading \"%s\": %s",
			    URLTools.unescape(path), Errors.describe(e)), e);
	} catch (SAXException e) {
	    this._logger.warn(String.format("Error reading \"%s\": %s",
		    URLTools.unescape(path), Errors.describe(e)));
    ...
    /**
     * Indicates whether the given filename is already loaded for any bot at
     * all.
     * 
     * @param filename
     ...
     * @param filename
     */
    abstract protected void associateBotIDWithFilename(String botid,
	    URL filename);

    /**
     * Allows a concrete version of this class to decide whether or not a file
     * needs to be loaded; this is useful for {@link DBGraphmapper}, which
     * doesn't need to load a file again if it hasn't changed since it was last
     * loaded.
     * 
     * @param path
     ...
     *             if the given path has not already been loaded, or if it has
     *             been loaded for the same botid
     */
    ...
    public void addCategory(String pattern, String that, String topic,
	    String template, Bot bot, URL source) {
	...
	this._logger.info(String.format("%,d categories loaded so far.",
		this._totalCategories));
	add(_pattern, _that, _topic, template, bot, source);
    }

    protected abstract void add(String pattern, String that, String topic,
	    String template, Bot bot, URL source);
     ...
     * supplied is important: the first one (<code>existingTemplate</code>) is
     * processed as though it has already been stored in the Graphmaster, and
     ...
     * @param newTemplate
     *            the template which should be combined with the existing
     *            template
     * @return the combined result
     */
    @SuppressWarnings("unchecked")
    protected String combineTemplates(String existingTemplate,
	    String newTemplate) {
	...
	try {
	    existingDoc = new SAXBuilder().build(new StringReader(
		    existingTemplate));
	    ...
	    for (Content newContentItem : (List<Content>) newDoc
		    .getRootElement().getContent()) {
		newContent.add((Content) newContentItem.clone());
	    }
	} catch (JDOMException e) {
	    this._logger.error("JDOM exception when performing merge combine.",
		    e);
	    return existingTemplate;
	} catch (IOException e) {
	    this._logger
		    .error("IO exception when performing merge combine.", e);
	    return existingTemplate;
	}
	...
	if (firstNode instanceof Element) {
	    ...
	    if (firstElement.getName().equals(RandomProcessor.label)
		    && firstElement.getAttribute("synthetic") != null) {
		Element newListItem = new Element(RandomProcessor.LI,
			this._aimlNamespaceURI);
	...
	Element listItemForExisting = new Element(RandomProcessor.LI,
		this._aimlNamespaceURI);
	existingRoot.removeContent();
	listItemForExisting.addContent(existingContent);

	Element listItemForNew = new Element(RandomProcessor.LI,
		this._aimlNamespaceURI);
	listItemForNew.addContent(newContent);

	Element newRandom = new Element(RandomProcessor.label,
		this._aimlNamespaceURI);
	...
	try {
	    ...
	    this._logger.error("JDOM exception when performing merge append.",
		    e);
	    return existingTemplate;
	} catch (IOException e) {
    ...
    /**
     * Composes an input path as a list of tokens, given the components. Empty
     * components are represented with asterisks.
     * 
     * @param input
     * @param that
     * @param topic
     ...
     * @return the new path
     */
    protected List<String> composeInputPath(String input, String that,
	    String topic, String botid) {
	...
	return String.format("%,d total categories currently loaded.",
		this._totalCategories);
    }
	...
	try {
	    print(new PrintWriter(Filesystem.checkOrCreate(path,
		    "Graphmapper output")));
	} catch (FileNotFoundException e) {
