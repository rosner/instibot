 ...
 * <code>Nodemapper</code>s. These <code>Nodemapper</code> s map the branches
 * from each node. The branches are either single words or wildcards.
 * </p>
 * <p>
 * The root of the <code>Graphmaster</code> is a <code>Nodemapper</code> with
 * many branches, one for each of the first words of all the patterns. The
...
public class MemoryGraphmapper extends AbstractGraphmapper {
    ...
    /**
     * Creates a new <code>Graphmaster</code>, reading settings from the given
     * Core.
     * 
     * @param core
	...
	super(core);
	// this.NodemapperFactory = new
	// ObjectFactory<Nodemapper>(this._core.getSettings().getNodemapperImplementation());
	this.root = new TwoOptimalMemoryNodemapper();
    }
	...
	return isAlreadyLoaded(filename)
		&& this._urlCatalog.get(filename).contains(botid);
    }
    ...
    /**
     * @see org.aitools.programd.graph.AbstractGraphmapper#addForBot(java.net.URL,
     *      java.lang.String)
     */
    @Override
	...
	if (this._logger.isDebugEnabled()) {
	    this._logger.debug(String.format(
		    "Adding botid \"%s\" to all paths associated with \"%s\".",
		    botid, path));
	}
    ...
    /**
     * @see org.aitools.programd.graph.Graphmapper#addCategory(java.lang.String,
     *      java.lang.String, java.lang.String, java.lang.String,
     *      org.aitools.programd.Bot, java.net.URL)
     */
    @Override
    public void add(String pattern, String that, String topic, String template,
	    Bot bot, URL source) {
	...
	if (storedTemplate == null) {
		...
		if (this._noteEachMerge) {
		    this._logger
			    .warn(String
				    .format("Skipping path-identical category from \"%s\" which duplicates path of category from \"%s\": %s:%s:%s",
					    source, nodemapper.get(FILENAME),
					    pattern, that, topic));
		}
		break;
		...
		if (this._noteEachMerge) {
		    this._logger
			    .warn(String
				    .format("Overwriting path-identical category from \"%s\" with new category from \"%s\".  Path: %s:%s:%s",
					    nodemapper.get(FILENAME), source,
					    pattern, that, topic));
		}
		...
		if (this._noteEachMerge) {
		    this._logger
			    .warn(String
				    .format("Appending template of category from \"%s\" to template of path-identical category from \"%s\": %s:%s:%s",
					    source, nodemapper.get(FILENAME),
					    pattern, that, topic));
		}
		nodemapper.put(FILENAME, String.format("%s, %s",
			nodemapper.get(FILENAME), source));
		nodemapper.put(TEMPLATE,
			appendTemplate(storedTemplate, template));
		break;
		...
		if (this._noteEachMerge) {
		    this._logger
			    .warn(String
				    .format("Combining template of category from \"%s\" with template of path-identical category from \"%s\": %s:%s:%s",
					    source, nodemapper.get(FILENAME),
					    pattern, that, topic));
		}
		nodemapper.put(FILENAME, String.format("%s, %s",
     ...
     */
    protected Nodemapper add(String pattern, String that, String topic,
	    String botid, URL source) {
     ...
     */
    protected Nodemapper add(ListIterator<String> pathIterator,
	    Nodemapper parent, URL source) {
	...
	if (parent.containsKey(word)) {
	    ...
	    nodemapper = new TwoOptimalMemoryNodemapper();
     ...
     * @param botid
     * @return the match
     * @see org.aitools.programd.graph.Graphmapper#match(java.lang.String,
     *      java.lang.String, java.lang.String, java.lang.String)
	...
	Nodemapper result = match(this.root, this.root,
		composeInputPath(input, that, topic, botid), "",
		new StringBuilder(), match, Match.State.IN_INPUT,
		System.currentTimeMillis() + this._responseTimeout);
	if (result != null) {
	    return match;
	}
	throw new NoMatchException(String.format("%s:%s:%s:%s", input, that,
		topic, botid));
    }
     ...
     * @param match
     *            an object containing information about the match
     * @param matchState
     ...
     * @throws NoMatchException
     *             if match time expires
     */
    @SuppressWarnings("boxing")
    protected Nodemapper match(Nodemapper nodemapper, Nodemapper parent,
	    List<String> input, String wildcardContent, StringBuilder path,
	    Match match, Match.State matchState, long expiration)
	    throws NoMatchException {
	if (nodemapper == null) {
	    return null;
	}
	...
	// Halt matching if this nodemapper is higher than the length of the
	// input.
	if (input.size() < nodemapper.getHeight()) {
	    if (this._matchLogger.isDebugEnabled()) {
		this._matchLogger
			.debug(String
				.format("Halting match because input size %d < nodemapper height %d.%ninput: %s%nnodemapper: %s",
					input.size(), nodemapper.getHeight(),
					input.toString(), nodemapper.toString()));
	    }
	    return null;
	}
	...
	if (input.size() == 0) {
		...
		match.setFilenames(Arrays.asList(((String) nodemapper
			.get(FILENAME)).split(",")));
		return nodemapper;
	    }
	    // (otherwise...)
	 ...
	 * it didn't contain a _ at all. So let's see if it contains the head.
	 */
	if (nodemapper.containsKey(head)) {
	    /*
	     * Check now whether this head is a marker for the <that>, <topic>
	     * or <botid> segments of the path. If it is, save the contents of
	     * the thereby terminated path component, and then set the new match
	     * state variable accordingly.
	     */
	    boolean isMarker = false;
	    if (head.startsWith("<")) {
		match.setPathComponent(_matchState, path.toString()
			.toUpperCase());
		if (head.equals(THAT)) {
		    isMarker = true;
	    ...
	    nextNodemapper = match(head, // key
		    isMarker ? _matchState.preceding() : null, // target match
							       // state for
							       // wildcard
							       // content
		    nodemapper, // current nodemapper
		    tail, // current tail
		    !isMarker, // append new path? (only if this is not a
			       // marker)
		    wildcardContent, // current wildcard content (empty if this
				     // is a marker)
	 ...
	 * the head, nor *. However, if its parent is a wildcard, then the match
	 * continues to be valid and can proceed with the tail, the current
	 * path, and the star content plus the head as the new star.
	 */
	if (nodemapper.equals(parent.get(ASTERISK))
		|| nodemapper.equals(parent.get(UNDERSCORE))) {
	    nextNodemapper = match(nodemapper, // current nodemapper
		    parent, // current path
		    tail, // current tail
		    String.format("%s %s", wildcardContent, head), // head =
								   // wildcard
								   // content +
								   // head
		    path, // current path
	...
	/*
	 * If we get here, we've hit a dead end; this null value will be passed
	 * back up the recursive chain of matches, perhaps even hitting the
	 * high-level match method and causing a NoMatchException, though this
	 * is assumed to be the rarest occurence.
	 */
	return null;
    }

    /**
     * An internal method used for matching. This method <i>assumes</i> that
     * nodemapper.containsKey(key)!
     * 
     * @param key
     ...
     * @throws NoMatchException
     */
    protected Nodemapper match(String key, Match.State wildcardDestination,
	    Nodemapper nodemapper, List<String> tail, boolean appendToPath,
	    String currentWildcard, String newWildcard, StringBuilder path,
	    Match match, Match.State matchState, long expiration)
	    throws NoMatchException {
	...
	// Try to get a match with the tail and this new path (may throw
	// exception)
	Nodemapper result = match((Nodemapper) nodemapper.get(key), // newly
								    // matched
								    // nodemapper
		nodemapper, // current nodemapper as parent
		tail, // current tail
		newWildcard, // current wildcardContent
		appendToPath ? newPath : new StringBuilder(), // either the new
							      // path, or a
							      // blank one
		match, // match object
	...
	// match state.
	if (wildcardDestination != null
		&& wildcardDestination.compareTo(Match.State.IN_BOTID) < 0
    ...
    /**
     * @see org.aitools.programd.graph.Graphmapper#removeCategory(java.lang.String,
     *      java.lang.String, java.lang.String, org.aitools.programd.Bot)
     */
    public void removeCategory(String pattern, String that, String topic,
	    Bot bot) {
	Nodemapper nodemapper = null;
	try {
	    nodemapper = match(this.root, this.root,
		    composeInputPath(pattern, that, topic, bot.getID()), "",
		    new StringBuilder(), new Match(), Match.State.IN_INPUT,
		    System.currentTimeMillis() + this._responseTimeout);
	...
	if (nodemapper != null) {
	    ...
	    this._logger.error(String.format(
		    "Could not find category to remove (%s:%s:%s)", pattern,
		    that, topic, bot));
	}
    }
     ...
     * 
     * @param nodemapper
     *            the mapper for the nodemapper to remove
     */
    ...
    /**
     * @see org.aitools.programd.graph.Graphmapper#unload(java.net.URL,
