...
import org.aitools.programd.graph.MemoryGraphmapper;
...
import org.aitools.programd.predicates.InMemoryPredicateManager;
...
public class Core {
    ...
    private static String XML_CATALOG_URL = "catalog.xml";

    /** The location of the XMLResolver config file, as a string. */
    private static String XMLRESOLVER_CONFIG_URL = "XMLResolver.properties";
    ...
    /**
     * A general-purpose map for storing all manner of objects (by AIML
     * processors and the like).
     */
    private Map<String, Map<String, Object>> classStorage = new HashMap<String, Map<String, Object>>();

    /**
     * Initializes a new Core object with default settings and the given base
     * URL.
     * 
     * @param base
     *            the base URL to use
    ...
    /**
     * Initializes a new Core object with the settings from the given file and
     * the given base URL.
     * 
     * @param base
     *            the base URL to use
     * @param settings
     *            the path to the file with settings
     */
	...
	this._baseURL = base;
	this._xmlCatalog = URLTools.contextualize(this._baseURL,
		XML_CATALOG_URL);
	this._xmlresolverConfig = URLTools.contextualize(this._baseURL,
		XMLRESOLVER_CONFIG_URL);
    }
	...
	this._xmlParserFeatureSettings.put(
	...
	this._logger.info(String.format("Base URL for Program D Core: \"%s\".",
		this._baseURL));

	this._aimlProcessorRegistry = new AIMLProcessorRegistry();

	this._graphmapper = new MemoryGraphmapper(this);

	this._bots = new Bots();
	this._processes = new ManagedProcesses(this);

	// Get an instance of the settings-specified PredicateManager.
	this._predicateManager = new InMemoryPredicateManager(this);
	...
	if (pluginConfigURL != null) {
	    pluginConfigURL = URLTools.contextualize(this._baseURL,
		    pluginConfigURL);
	    try {
		if (pluginConfigURL.openStream() != null) {
		    this._pluginConfig = JDOM.getDocument(pluginConfigURL,
			    this._logger);
		}
	    } catch (IOException e) {
	...
	this._logger.info(String.format("Starting %s version %s [%s].",
		pkg.getSpecificationTitle(), pkg.getSpecificationVersion(),
	...
	try {
		...
		this._logger
			.warn("No bot config URL specified; no bots will be loaded.");
	    }
	...
	if (this._settings.allowJavaScript()) {
		...
		this._logger.error(new UnspecifiedParameterError(
	    ...
	    try {
		this._interpreter = (Interpreter) Class.forName(
			javascriptInterpreterClassname).newInstance();
	    } catch (Exception e) {
		this._logger
			.error("Error while creating new instance of JavaScript interpreter.",
				e);
	    }
	} else {
	...
	if (this._status == Status.READY) {
	    ...
	    List<String> sentenceList = bot.sentenceSplit(bot
		    .applyInputSubstitutions(input));
	...
	if (!this._bots.containsKey(id)) {
	    this._logger.warn("Bot \"" + id
		    + "\" is not loaded; cannot unload.");
	    return;
	}
	...
	throw new NullPointerException(
		"The Core's Graphmapper object has not yet been initialized!");
    }
    ...
    /**
     * Gets an object from the class storage, using the given classname to look
     * up the map for the class, then the given key to retrieve the object. If
     * the object is not found, then the given defaultObject is stored with the
     * appropriate key, so it will be there next time.
     * 
     * @param <T>
     *            the type of object to retrieve
     * @param classname
     *            the classname from whose map to retrieve
     * @param key
     *            the key to retrieve
     * @param defaultObject
     *            default object to use if none is found
    ...
    public String getInternalResponse(String input, String userid,
	    String botid, TemplateParser parser) {
	...
	List<String> thatSentences = bot.sentenceSplit(this._predicateManager
		.get("that", 1, userid, botid));
	String that = InputNormalizer.patternFitIgnoreCase(thatSentences
		.get(thatSentences.size() - 1));
	...
	assert stackSize == thats.size() && thats.size() == topics.size() : String
		.format("%d inputs, %d thats, %d topics", stackSize,
			thats.size(), topics.size());
	...
	if (stackSize > 1) {
		...
		String infiniteLoopInput = parser.getCore().getSettings()
			.getInfiniteLoopInput();
		if (that.equalsIgnoreCase(comparisonThat)
			&& topic.equalsIgnoreCase(comparisonTopic)) {
			...
			this._matchLogger.warn(String.format(
				"Infinite loop detected; substituting \"%s\".",
				infiniteLoopInput));
		    }
		}
	    }
    ...
    protected List<String> getReplies(List<String> sentenceList, String userid,
	    String botid) {
	...
	List<String> replies = Collections.checkedList(new ArrayList<String>(
		sentenceList.size()), String.class);
	...
	if (thatSentences.size() > 0) {
	    that = InputNormalizer.patternFitIgnoreCase(thatSentences
		    .get(thatSentences.size() - 1));
	}

	if (that == null || "".equals(that)
		|| that.equals(this._predicateEmptyDefault)) {
	    that = "*";
	}

	String topic = InputNormalizer
		.patternFitIgnoreCase(this._predicateManager.get("topic",
			userid, botid));
	if ("".equals(topic) || topic.equals(this._predicateEmptyDefault)) {
	    topic = "*";
	}
	...
	this._avgResponseTime = (float) this._totalTime
		/ (float) this._responseCount;
	if (this._matchLogger.isDebugEnabled()) {
	    this._matchLogger.debug(String.format(
		    "Response %d in %dms. (Average: %.2fms)",
		    this._responseCount, time, this._avgResponseTime));
	}
     ...
     */
    protected String getReply(String input, String that, String topic,
	    String userid, String botid) {
	// Push the input onto the <input/> stack.
	this._predicateManager.push("input", input, userid, botid);

	// Create a new TemplateParser.
	TemplateParser parser = new TemplateParser(input, that, topic, userid,
		botid, this);

	String reply = getMatchResult(input, that, topic, userid, botid, parser);
	if (reply == null) {
	    this._logger.error("getMatchReply generated a null reply!",
		    new NullPointerException("Null reply."));
	    return "";
	}
     ...
     */
    protected String getMatchResult(String input, String that, String topic,
	    String userid, String botid, TemplateParser parser) {
	// Show the input path.
	if (this._matchLogger.isDebugEnabled()) {
	    this._matchLogger.debug(String.format("[INPUT (%s)] %s:%s:%s:%s",
	...
	try {
	    match = this._graphmapper.match(
		    InputNormalizer.patternFitIgnoreCase(input), that, topic,
		    botid);
	...
	if (match == null) {
	    this._logger.warn(String.format("No match found for input \"%s\".",
		    input));
	    return "";
	}

	if (this._matchLogger.isDebugEnabled()) {
	    this._matchLogger.debug(String.format("[MATCH (%s)] %s (\"%s\")",
		    userid, match.getPath(), match.getFileNames()));
	}
	...
	try {
	    reply = parser.processResponse(template, match.getFileNames()
		    .get(0));
	} catch (Throwable e) {
     ...
     */
    protected void logResponse(String input, String response, String userid,
	    String botid) {
	/*
	 * NOTA BENE: This is a very specific workaround for a problem with the
	 * log4j JDBCAppender. It appears that the appender fails to maintain
	 * the database connection after some period of time, and thus stops
	 * working. Here, we catch the exception thrown in this case, and force
	 * log4j to reinitialize the appender. This is horribly specific and
	 * should go away as soon as possible. - 2006-03-29, NB
	 */
	try {
	    this._logger.callAppenders(new ChatLogEvent(botid, userid, input,
		    response));
	} catch (Exception e) {
	    this._logger
		    .error("A known bug with log4j has been encountered.  Attempting to reset logging configuration. This may or may not work.",
			    e);
	...
	return this._responseCount
		/ ((System.currentTimeMillis() - this._startTime) / 3600000.00f);
    }
}
